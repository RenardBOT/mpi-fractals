...
// Fonction d'écriture d'un fichier BMP à partir des structures définies dans save_bmp.h
...
{   
    ...
    if (!output_file)
    {
        ...
        printf("Attention à bien se situer dans le dossier racine, et de lancer la commande\n");
        printf("mpiexec -np <nb-taches> bin/mandelbrot [OPTIONS]\n");
        MPI_Finalize();
        exit(EXIT_FAILURE);
    } 
